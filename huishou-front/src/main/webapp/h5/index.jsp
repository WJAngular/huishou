...
.picScroll .bd li img {
	width: 90px;
	height: 90px;
	background: url(<%=request.getContextPath()%>/images/m/partner/loading.gif)
...
.picScroll .prev, .picScroll .next {
	...
	width: 18px;
	height: 18px;
	background: url(<%=request.getContextPath()%>/images/m/partner/pisScrollIcons.gif)
		-6px -7px no-repeat;
	overflow: hidden;
...
<div class="ui  fixed menu secondary" style="background-color: white;">
	...
	<div class="right menu">
		...
		<c:if test="${not empty user }">
			<div class="item">
				<a onclick="to_page('/user/home')">${user.username }</a><a>&nbsp;|&nbsp;</a> <a onclick="logout()">退出</a>
			</div>
		</c:if>
							...
							<c:forEach items="${brand.product }" var="product" end='5'>
								<div class="ui  card" onclick="product(${product.id })">
									...
									<div class="center floated content" style="text-align: center;">
										<h4>${product.name }</h4>
										<h6>
											回收最高价格 <a style="color: orange">￥${product.recycleMaxMoney} </a>
										</h6>
...
<!-- 本例主要代码 Start ================================ -->
<div id="picScrollA" class="picScroll">
	<div class="hd">
		<span class="next"></span>
		<ul></ul>
		<span class="prev"></span>
		<h3 style="margin-top: 10px;">回收评价</h3>
	</div>
	<div class="bd">
		<ul>
			<c:forEach items="${commentInfos }" var="comment" varStatus="c" begin="0" end="2">
				<li>
		...
		<ul>
			<c:forEach items="${commentInfos }" var="comment" varStatus="c" begin="3" end="5">
				<li>
					<div class="ui comments">
						<div class="comment">
							<div class="content">
		...
		<ul>
			<c:forEach items="${commentInfos }" var="comment" varStatus="c" begin="6" end="8">
				<li>
					<div class="ui comments">
						<div class="comment">
							<div class="content">
...
<div class="ui internally celled grid center aligned" style="background-color: #F8F8F9;"></div>



<div class="ui container column " style="font-size: 1em;">
	<h4 class="ui horizontal divider blue header">
		<i class="newspaper icon"></i> 最新资讯
	</h4>
	<div class="ui relaxed divided list">
		...
		<div class="item">
			...
			<div class="content">
				<a class="header" href="${news.newsUrl }" target="_blank">${news.title }</a></div>
			   <div class="description" style="font-size: 12px;">${news.summary }</div>
			</div>
		</div>
	</c:forEach>
	</div>
...
<div class="ui internally celled grid center aligned" style="background-color: #f4f4f4;">
	...
	<div class="row">
		<div class="five wide column" onclick="to_page('/h5/info/about/view.jsp')">联系我们</div>
		<div class="five wide column" onclick="javascript:window.open('http://wpa.qq.com/msgrd?v=3&uin=1552107066&site=qq&menu=yes')">QQ客服</div>
		<div class="five wide column" onclick="popLoginModal()">我的订单</div>
	</div>
	<div class="row">
		<div class="five wide column" onclick="to_page('/h5/other/newproduct/view.jsp')">提交型号</div>
		<div class="five wide column" onclick="to_page('/h5/other/bigrecycle/view.jsp')">量大回收</div>
		<div class="five wide column" onclick="to_page('/h5/other/join/view.jsp')">我要加盟</div>
	</div>
				...
				<form class="ui form mini" id="loginForm">
					...
					<div class="field">
						<div class="ui mini icon input">
							<input type="text" placeholder="请输入图片验证码" id="code" name="imgCode"> <img id="codeImg" src="<%=request.getContextPath()%>/system/randomCode" class="ui image" onclick="javascript:this.src='<%=request.getContextPath()%>/system/randomCode?' + new Date().getTime();">
						</div>
					</div>
